...
While Nosto crawls sites to replicate the product catalog, it is unable to do so on SPAs. In order to synchronise your product catalog with Nosto, you'll need to [leverage the product API](Updating-products-using-the-Products-API.md) to keep the Nosto catalog in sync.
...
When a visitor logs in customer information _should_ be passed. If the customer isn't logged in, this can be omitted. Similar to setting the [cart contents](SPA/Basics#setting-the-cart), customer data is only sent to Nosto when an [action](an [action](Session-API---Terminology#action)) is performed.
