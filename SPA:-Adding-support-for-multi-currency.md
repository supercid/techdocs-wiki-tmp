...
The [product-update API](Updating-products-using-the-Products-API.md) calls must be amended to denote the primary currency code of the product. Typically, most retailers have a primary currency which is the default currency of the inventory.
...
In order to send the exchange rate multipliers to Nosto, you will need to use [our exchange-rates API](Updating-Rates-using-the-Rates-API.md). Below is a small snippet of what the payload looks like.
